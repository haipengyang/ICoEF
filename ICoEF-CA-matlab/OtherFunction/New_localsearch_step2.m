function solutions = New_localsearch_step2(n, solutions, Y, C, F, Adj, popsize)
...
for i = 1 : length(num)
    change_factor(i) = -(solutions(num(i), n + 2)) / solutions(num(i), n + 1);
...
[fail_list1, ~, ~] = CascadeModel3(solutions(change_index(1), 1 : n), Y, C, F, Adj);  %第一个参考点的失效范围
...
if length(change_factor_sort) > 1
    ...
    if temp_change_index3 ~= change_index
        ...
        [fail_list2, ~, ~] = CascadeModel3(solutions(change_index(2), 1 : n), Y, C, F, Adj);  %第一个参考点的失效范围
    end
...
for i = 1 : length(change_solution_index)
    ...
    if length(change_index) == 2
        if (value ~= change_index(2)) && (solutions(value, n + 2) <= solutions(change_index(2), n + 2))
            [temp_flag_2, temp_change] = Max_change(n, solutions(change_index(2), :), fail_list2, solutions(value, :), Y, C, F, Adj);
    ...
    if (value ~= change_index(1)) && (temp_flag == 0)
        [temp_flag_2, temp_change] = Max_change(n, solutions(change_index(1), :), fail_list1, solutions(value, :), Y, C, F, Adj);
        temp_flag = temp_flag_2;
        if temp_flag == 1
            new_change = [new_change; temp_change];
            local_search_flag = local_search_flag + 1;
