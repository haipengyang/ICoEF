...
NC = max(max(short_paths));  % Number of network divisions
F = Load(k, a, n);  % 节点负载
C = Capacity(F, c, n);  % 节点容量
Y = ResidualCapacity(F, C, n);  % 节点剩余容量
impact = Potential(Y, C, F, n, Adj);  % 节点潜力值
max_num = Kmin(n, Adj, impact, Y, C, F);  % Kmin
...
divide_k = cell(divide_Num, 1);  % 子网络节点度
divide_F = cell(divide_Num, 1);  % 子网络节点负载
divide_C = cell(divide_Num, 1);  % 子网络容量
divide_Y = cell(divide_Num, 1);  % 子网络剩余容量
for i = 1 : divide_Num
    divide_k{i} = Degree(divide_Adj{i});
    divide_F{i} = Load(divide_k{i}, a, divide_n(i));
    divide_C{i} = Capacity(divide_F{i}, c, divide_n(i));
    divide_Y{i} = ResidualCapacity(divide_F{i}, divide_C{i}, divide_n(i));
end
[divide_change, divide_change_sum] = Change_degree(k, divide_Adj_index, divide_k, divide_Num, divide_n);   % [全局到局部的节点度的变化,局部网络度变化和]
...
for Time = 6 : 15
    ...
    parfor i = 1 : init_unit
        fitnessSolution(i) = -CascadeModel(solution(i, 1 : n), Y, C, F, Adj);   % 评价全局网络总失效节点数
    end
    ...
    for d = 1 : divide_Num
        ...
        parfor i = 1 : popsize
            fitnessList_temp(i) = FitnessLocal(population{d}(i, 1 : divide_n(d)), divide_Adj{d}, divide_change{d}, divide_n(d), divide_change_sum(d), divide_F{d}, divide_C{d}, divide_Y{d});   % 评价网络失效总结点数和局部损失信息
        end
    ...
    while generation ~= iteration        
        ...
        if g_flag == 1
            ...
            parfor i = 1 : popsize
                fitnessSolution(i) = -CascadeModel(Offspring(i, 1 : n), Y, C, F, Adj);   % 评价Pair_wise
            end
            for i = 1 : popsize  % 将每个个体的代价和攻击效果加在个体编码之后
            ...
            parfor i = 1 : length(newnum{generation})
                [PP(i, :),  search_success_flag(i), checks(i), wchecks(i)] = New_localsearch_step1(temp_solutions(newnum{generation}(i), :), Y, C, F, n, Adj, k, impact);
            end
            ...
            % 局部搜索，阶段二，性能最大化替换
            solution = New_localsearch_step2(n, temp_solutions, Y, C, F, Adj, popsize);
            ...
            if (global_count == 5) && (local_stop_flag == 0)
                ...
                for d = 1 : divide_Num
                    ...
                    parfor i = 1 : popsize
                        fitnessList_temp(i) = FitnessLocal(population{d}(i, 1 : divide_n(d)), divide_Adj{d}, divide_change{d}, divide_n(d), divide_change_sum(d), divide_F{d}, divide_C{d}, divide_Y{d});   % 评价Pair_wise和局部损失信息
                    end
                    for i=1 : popsize  % 将每个个体的代价和攻击效果加在个体编码之后
                        population{d}(i, divide_n(d) + 1) = failNumList_temp(i);   % 函数值
                        population{d}(i, divide_n(d) + 2) =  fitnessList_temp(i);
                ...
                if local_stop_flag == 1
                    ...
                    end_flag = 1;
                    for hv_time = 1 : 5
                        if HV1(Time, generation - hv_time) == 0
                            end_flag = 0;
                            break;
                        end
                        if hv_result ~= HV1(Time, generation - hv_time)
                            end_flag = 0;
                            break;
                        end
                    end
            ...
            for d =1 : divide_Num  % 顺序循环处理局部子种群
                ...
                parfor i = 1 : popsize
                    fitnessList_temp(i) = FitnessLocal(Offspring(i, 1 : divide_n(d)), divide_Adj{d}, divide_change{d}, divide_n(d), divide_change_sum(d), divide_F{d}, divide_C{d}, divide_Y{d});   
                end
                for i=1 : popsize  % 将每个个体的代价和攻击效果加在个体编码之后
        ...
        Ti=roundn(toc, -2);
        fprintf('ICoEF-CA-matlab, 第%2s次, 第%2s轮, %5s问题, 已完成%4s, 耗时%5s秒\n', num2str(Time), num2str(generation), num2str(iii), num2str(roundn(generation / iteration * 100, -1)), num2str(Ti));
        T(Time)=Ti;
        if end_flag == 1
            break;
        end
    end
    ParetoFront{Time}=unique(P, 'rows');
    
    datapathHV1 = 'Result\HV1';                                         %储存的文件夹路径
    datapathParetoFront = 'Result\ParetoFront';
    datapathT = 'Result\T';
    %dirpath=[datapath,'\','filename{i}',num2str(i),'\',name];
    datanameHV = ['ICoEF-CA-matlab_100g_HV_T',num2str(Time),'_', pathname];
    datanameParetoFront = ['ICoEF-CA-matlab_100g_ParetoFront_T',num2str(Time),'_', pathname];
    datanameT = ['ICoEF-CA-matlab_100g_T_T',num2str(Time),'_', pathname];
