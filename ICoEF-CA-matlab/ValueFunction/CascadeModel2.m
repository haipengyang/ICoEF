function A = CascadeModel2(B, Y, C, F, Adj, k)
%CASCADEMODEL2 级联失效模型2
%   B为1到n的0数组，Y为节点剩余容量，C为节点容量，F为节点负载，Adj为数据集，k为节点度
%   找出失效节点，在局部搜索中使用，计算每个节点被攻击后，网络的总失效节点数，不同之处在于是在剩余网络上进行计算
t = 1;                                                            %初始级联
...
%         d5{i} = d1;                                                  %储存累计的d1，即负载将会增加的点,元胞数组的长度等于上一次级联的失效点的个数
        if ~isempty(d1)                                           %向邻居节点分配负载,分配比例与负载有关
            ...
            for j = 1 : length(d1)
                ...
                F1{t}(d1(j)) = F1{t}(d1(j)) + F1{t - 1}(e{t - 1}(i)) * k(d1(j)) / sum;
            end
        end
    end
